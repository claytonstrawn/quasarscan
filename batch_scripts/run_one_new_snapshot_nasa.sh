...
if (($n == '0')); then
    python quasarscan/create_qso_endpoints.py $simname $filename $ionlist
fi
mpiexec -np $procs python quasarscan/get_coldens.py quasarscan/output/${simname}coldensinfo/${n}_of_384-${ionlist}_z${z}.txt $procs p
