...
class NeedAgoraError(Exception):
...
yt_dstype_names = {'art':'art','ramses':'ramses',\
                   'gizmo':'gadget_hdf5','gadget':'gadget_hdf5',\
                   'gear':'gadget_hdf5','enzo':'enzo_packed_3d',\
                   'tipsy':'tipsy','changa':'tipsy',\
                   'mockstreams':'stream'}
...
def ytload(path,fullname,redshift):
    simname,version,code,simnum = fullname.split('_')
    if simname == 'AGORA':
        try:
            import agora_analysis
            from agora_analysis.field_setup.main import add_gas_fields,\
                                                    add_metallicity_fields,\
                                                    add_temperature_fields,\
                                                    override_default_fields
        except ImportError:
            raise NeedAgoraError('agora_analysis must be installed to read codes with the name "AGORA"')

        agora_name = '%s_%s_%s'%(simname,code,'CR')
        snap = agora_analysis.AgoraSnapshot(agora_name,redshift)
        snap.load_snapshot()
        agora_analysis.field_setup.main.add_gas_fields(snap)
        agora_analysis.field_setup.main.add_metallicity_fields(snap)
        agora_analysis.field_setup.main.add_temperature_fields(snap)
        agora_analysis.field_setup.main.override_default_fields(snap)
        return snap.ds
    if code == 'art':
...
def fields_to_keep_in_sightline(code,ions,add_pi_fracs=True):
    fields_to_keep = [('gas',"density"),('gas',"mass"),\
                      ('gas',"temperature"),('gas',"radial_velocity"),\
                      ('gas','normed_radial_distance'),('gas','relative_velocity')]
...
def check_redshift(ds,fullname = None,outputfilename=None,redshift = None,tolerance = 0.05):
    ...
    if code == 'mockstreams':
        ...
        assert np.abs(ds.current_redshift - redshift)<tolerance,\
        "expected simulation to be at redshift %s,"%redshift+\
        " but was at redshift %s!"%ds.current_redshift
...
def load_and_setup(path,fullname,ions=None,add_pi_fracs=True,redshift = None):
    code = fullname.split("_")[2]
    if code not in codes:
        print("load_and_setup was not prepared for the code %s!"%code)
        print("Please edit that file first.")
        raise NotImplementedError
    ds = ytload(path,fullname,redshift)
    try:
        assert yt_dstype_names[code] == ds.dataset_type
    except:
        print('the code stored at: %s, as a code of type %s,'%(path,code,)+\
        ' should be of type %s, but it is of type %s'%(yt_dstype_names[code],ds.dataset_type))
