...
#  (modified in August 2018 to be used for CGM/Clumps papers)
#
...
draw_cellsize_map                = 1#3           # 0/1/2/3     = OFF/ON/ON now with resolution map where particle_size is defined as [M/rho]^(1/3) for SPH/ON with both cell_size and resolution maps
...
draw_PDF                         = 2###3         # 0/1/2/3     = OFF/ON/ON with constant pressure/entropy lines/ON with additional annotations such as 1D profile from a specific code (CHANGA)
draw_pos_vel_PDF                 = 1##4          # 0/1/2/3/4   = OFF/ON/ON with 1D profile/ON also with 1D dispersion profile/ON also with separate 1D vertical dispersion profiles
...
dataset_num                      = 1             # 1/2         = 1st dataset(Calibration step #1)/2nd dataset(Calibration step #2) for AGORA Paper CGM/Clumps
yt_version_pre_3_2_3             = 0             # 0/1         = NO/YES to "Is the yt version being used pre yt-dev-3.2.3?"
times                            = [19]      # 
figure_width                     = 15            # in kpc
...
mean_dispersion_radius_range     = [2]       # in kpc      = Used if add_mean_fractional_dispersion = 1, for draw_pos_vel_PDF, draw_star_pos_vel_PDF, draw_rad_height_PDF, draw_radius_DF, draw_star_radius_DF
mean_dispersion_height_range     = [0]      # in kpc      = Used if add_mean_fractional_dispersion = 1, for draw_height_DF
mean_dispersion_time_range       = [50]     # in Myr      = Used if add_mean_fractional_dispersion = 1, for draw_SFR
mean_dispersion_density_range    = [1e-25]# in g/cm3    = Used if add_mean_fractional_dispersion = 1, for draw_density_DF
...
file_location = ['/project/projectdirs/agora/paper_CGM/Cal_step2/']#,'/project/projectdirs/agora/paper_CGM/Cal_step2/']
codes = ['ART-I','ENZO','RAMSES','GEAR','GIZMO']
#codes = ['ART-I','ENZO','RAMSES','GADGET-3','GEAR','GIZMO']
filenames = [[['/project/projectdirs/agora/ART_I/Cosmo_v8/10MpcBox_csf512_a0.050.d'],[file_location[0]+'ENZO/DD0008/DD0008'],['/project/projectdirs/agora/RAMSES/Cosmo_run_v2/output_00007/info_00007.txt'],[file_location[0]+'GEAR/v2/own/snapshot_0047.hdf5'],[file_location[0]+'GIZMO/cos-mech/output/snapshot_006.hdf5']]]
#filenames = [[[file_location[0]+'ART_I/10MpcBox_csf512_a0.080.d'],[file_location[0]+'ENZO/RD0017/RD0017'],[file_location[0]+'RAMSES/output_00011/info_00011.txt'],[file_location[0]+'GADGET-3/v2/z10/snapshot_021.0.hdf5'],[file_location[0]+'GEAR/v2/own/snapshot_0070.hdf5'],[file_location[0]+'GIZMO/cos-mech/output/snapshot_017.hdf5']]]


time_text=[19,19,19,19,19]
...
#			timestamp = ''
#		else:
		timestamp = filenames_entry[code][time][filenames_entry[code][time].rfind('a'):filenames_entry[code][time].rfind('.')] 
		...
		pf = load(filenames_entry[code][time], unit_base = None , n_ref=n_ref, over_refine_factor=over_refine_factor)
#gadget_default_unit_base, n_ref=n_ref, over_refine_factor=over_refine_factor)
		print(pf.field_list)
	elif codes[code] == 'GEAR':
		pf = load(filenames_entry[code][time], unit_base = gadget_default_unit_base, n_ref=n_ref, over_refine_factor=over_refine_factor)
                print(pf.field_list)
...
#                        print(ad[PartType_Star_to_use])
			...
			MassType_to_use = "Masses"
                        ...
                        MassType_to_use = "Masses"
                        FormationTimeType_to_use ="StarFormationTime"
                elif codes[code] == 'ENZO':
                        PartType_Gas_to_use = "gas"
                        PartType_StarBeforeFiltered_to_use = "all"
...
#			pf.add_particle_filter(PartType_Star_to_use)
		...
		if codes[code] == "ART-I" or codes[code] == "ART-II" or codes[code] == "ENZO"  or codes[code] == "RAMSES":
			...
			pf.add_field((PartType_Gas_to_use, "Density_2"), function=_Density_2, take_log=True, particle_type=True, display_name="Density", units="g/cm**3") 
			def _Temperature_2(field, data):
				return data[(PartType_Gas_to_use, "Temperature")].in_units('K')
			pf.add_field((PartType_Gas_to_use, "Temperature_2"), function=_Temperature_2, take_log=True, particle_type=True, display_name="Temperature", units="K") 
			def _Mass_2(field, data):
				return data[(PartType_Gas_to_use, MassType_to_use)].in_units('Msun')
			pf.add_field((PartType_Gas_to_use, "Mass_2"), function=_Mass_2, take_log=True, particle_type=True, display_name="Mass", units="Msun")				
			if draw_metal_map >= 1 or draw_metal_PDF == 1:
				def _Metallicity_2(field, data):
			...
			pf.add_field((PartType_Gas_to_use, "Density_2_minus_analytic"), function=_Density_2_minus_analytic, take_log=False, particle_type=True, display_name="Density Residual", units="g/cm**3") 

		####################################
		#      MAIN ANALYSIS ROUTINES      #
		####################################
                ad=pf.all_data()
                print(ad[('gas',"velocity_z")].max().in_units('km/s'),ad[('gas',"velocity_z")].min().in_units('km/s'))
#                print(ad[('PartType1', 'Masses')].max().in_units('Msun'),ad[('PartType1', 'Masses')].min().in_units('Msun'))
#                print(ad[('PartType1', 'Masses')].max().in_units('g'),ad[('PartType1', 'Masses')].min().in_units('g'))
#                print(ad[('PartType0', 'Masses')].max().in_units('Msun'),ad[('PartType0', 'Masses')].min().in_units('Msun'))
#                print(ad[('PartType0', 'Masses')].max().in_units('g'),ad[('PartType0', 'Masses')].min().in_units('g'))
		...
		sp = pf.sphere(cen, (300,'kpc')) 
		cen2 = sp.quantities.center_of_mass(use_gas=True, use_particles=False).in_units("kpc")
		sp = pf.sphere(cen2, (60,'kpc')) 
                cen1=sp.argmax(("gas", "density"))
		sp = pf.sphere(cen1, (30,'kpc'))
		cen2 = sp.quantities.center_of_mass(use_gas=True, use_particles=False).in_units("kpc")
		sp2 = pf.sphere(cen2, (1.0, "kpc"))
		cen3 = sp2.quantities.max_location((PartType_Gas_to_use, "density"))
		...
		if draw_density_map == 1: 
			...
			if add_nametag == 1:
				at = AnchoredText("%s z=%s" % (codes[code],time_text[code]), loc=2, prop=dict(size=6), frameon=True)
		...
		if draw_star_map == 1: 
			for ax in range(1, 3):  
				p27 = yt.ParticleProjectionPlot(pf, ax, (PartType_Star_to_use, "particle_mass"), center = center, data_source=proj_region, width = (figure_width, 'kpc'), weight_field = None, fontsize=9)
...
#  							  finder_kwargs={'link': 0.02, 'dm_only': False, 'ptype': "all"})
			...
			if draw_star_clump_stats >= 2:
				for ax in range(1, 3):  
					p271 = yt.ParticleProjectionPlot(pf, ax, (PartType_Star_to_use, "particle_mass"), center = center, data_source=proj_region, width = (figure_width, 'kpc'), weight_field = None, fontsize=9)
				...
				for ax in range(1, 3):  
					p272 = yt.ParticleProjectionPlot(pf, ax, (PartType_Star_to_use, "particle_mass"), center = center, data_source=proj_region, width = (figure_width, 'kpc'), weight_field = None, fontsize=9)
		...
		if draw_SFR_map >= 1:
			...
			if add_nametag == 1:
				...
				at2 = AnchoredText("%s z=%s" % (codes[code],time_text[code]), loc=2, prop=dict(size=6), frameon=True)
		...
		if draw_PDF >= 1:
			...
			if codes[code] == "ART-I" or codes[code] == "ART-II" or codes[code] == "ENZO"  or codes[code] == "RAMSES":
				...
				p3.set_cmap((PartType_Gas_to_use, "Mass_2"), my_cmap2)
                                p3.set_log("Temperature_2", True)
                                p3.set_log("Density_2", True)
			...
			if add_nametag == 1:
				at = AnchoredText("%s z=%s" % (codes[code],time_text[code]), loc=3, prop=dict(size=10), frameon=True)
...
#					slc.save()
			...
			if add_nametag == 1:
				at = AnchoredText("%s z=%s" % (codes[code],time_text[code]), loc=4, prop=dict(size=10), frameon=True)
...
#			p41.set_unit((PartType_Star_to_use, "particle_mass"), 'Msun') # Neither this nor above works without such change
		...
		if draw_rad_height_PDF >= 1:
			...
			if add_nametag == 1:
				at = AnchoredText("%s z=%s" % (codes[code],time_text[code]), loc=1, prop=dict(size=10), frameon=True)
...
#				p6 = ProfilePlot(sp, (PartType_Gas_to_use, "Density_2"),  (PartType_Gas_to_use, "Mass_2"), weight_field=None, n_bins=50, x_log=True, accumulation=True)
	...
	# SAVE FIGURES
	if draw_density_map == 1:
		fig_density_map[time].savefig("Sigma_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
	if draw_temperature_map == 1:
		fig_temperature_map[time].savefig("Temp_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
	if draw_cellsize_map == 1 or draw_cellsize_map == 3:
		fig_cellsize_map[time].savefig("Cell_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
	if draw_cellsize_map == 2 or draw_cellsize_map == 3: 
		fig_cellsize_map_2[time].savefig("Resolution_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
	if draw_elevation_map == 1:
		fig_elevation_map[time].savefig("Elevation_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
	if draw_metal_map >= 1:
		fig_metal_map[time].savefig("Metal_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
	if draw_zvel_map == 1:
		fig_zvel_map[time].savefig("z-vel_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
	if draw_star_map == 1:
		fig_star_map[time].savefig("Star_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
	if draw_star_clump_stats >= 1:
		if draw_star_clump_stats >= 2:
			fig_star_map_2[time].savefig("Star_with_clumps_HOP_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)		
			fig_star_map_3[time].savefig("Star_with_clumps_FOF_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)		
...
#			plt.subplot(1,2,star_clump_stats_i, aspect=0.25)
			...
			for code in range(len(codes)):
				if (star_clump_stats_i == 1 and (codes[code] == "ART-I" or codes[code] == "ART-II"  or codes[code] == "ENZO" or codes[code] == "RAMSES")) or \
				   (star_clump_stats_i == 2 and (codes[code] == "CHANGA" or codes[code] == "GASOLINE" or codes[code] == "GADGET-3" or codes[code] == "GEAR" or codes[code] == "GIZMO")):
		...
		plt.savefig("star_clump_stats_HOP_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
		plt.clf()
...
#			plt.ylim(-0.1, 30)
			...
			ltext = leg.get_texts()
			plt.setp(ltext, fontsize='xx-small')
		plt.savefig("star_clump_stats_HOP_cumulative_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
		plt.clf()
...
#			plt.subplot(1,2,star_clump_stats_i, aspect=0.25)
			...
			ltext = leg.get_texts()
			plt.setp(ltext, fontsize='xx-small')
		plt.savefig("star_clump_stats_FOF_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
		plt.clf()
		# Reiterate for cumulative plots
		fig = plt.figure(figsize=(8, 4))
		gridspec.GridSpec(1, 2)
...
#			plt.ylim(-0.1, 30)
			...
			ltext = leg.get_texts()
			plt.setp(ltext, fontsize='xx-small')
		plt.savefig("star_clump_stats_FOF_cumulative_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
		plt.clf()
	if draw_SFR_map >= 1:
		fig_degr_density_map[time].savefig("degraded_Sigma_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
		fig_degr_sfr_map[time].savefig("degraded_SFR_map_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
		if draw_SFR_map == 2:
...
#				    color='k', linestyle ='-.', linewidth=1, alpha=0.7) # SFR surface density cutoff due to limited star particle mass resolution
			plt.savefig("K-S_local_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
			plt.clf()
	if draw_PDF >= 1:
		if draw_PDF == 3:
...
#				line_profile_ref = ln.Line2D(p35.profiles[0].x.in_units('g/cm**3'), p35.profiles[0]["temperature"].in_units('K'), linestyle="--", linewidth=2, color='k', alpha=0.7)
		...
		fig_PDF[time].savefig("PDF_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
	if draw_pos_vel_PDF >= 1:
		fig_pos_vel_PDF[time].savefig("pos_vel_PDF_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
		if draw_pos_vel_PDF >= 2:
			plt.clf()
...
#				plt.text(12, 0.3, "mean dispersion = %.3f or %.3f dex " % (mean_fractional_dispersion, mean_fractional_dispersion_in_dex), ha="center", va="center", size=8, bbox=dict(boxstyle="round", fc="w", ec="0.5", alpha=0.9))
			plt.savefig("pos_vel_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
			plt.clf()
		if draw_pos_vel_PDF >= 3:
			plt.clf()
#			plt.subplot(111, aspect=0.064)
			...
			if add_mean_fractional_dispersion == 1:
				...
				mean_fractional_dispersion_in_dex = np.log10(1.+mean_fractional_dispersion)
				plt.annotate("Mean fractional dispersion for %d < r < %d kpc = %.3f (%.3f dex) " % (mean_dispersion_radius_range[0], mean_dispersion_radius_range[1], mean_fractional_dispersion, mean_fractional_dispersion_in_dex), xy=(0.35, 0.08), size=10, xycoords='axes fraction')
			plt.savefig("pos_disp_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
...
#			plt.subplot(111, aspect=0.064)
			...
			if add_mean_fractional_dispersion == 1:
				...
				mean_fractional_dispersion_in_dex = np.log10(1.+mean_fractional_dispersion)
				plt.annotate("Mean fractional dispersion for %d < r < %d kpc = %.3f (%.3f dex) " % (mean_dispersion_radius_range[0], mean_dispersion_radius_range[1], mean_fractional_dispersion, mean_fractional_dispersion_in_dex), xy=(0.35, 0.08), size=10, xycoords='axes fraction')
			plt.savefig("pos_disp_vert_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
			plt.clf()
	if draw_star_pos_vel_PDF >= 1:
		fig_star_pos_vel_PDF[time].savefig("star_pos_vel_PDF_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
		if draw_star_pos_vel_PDF >= 2:
			plt.clf()
#			plt.subplot(111, aspect=0.04)
			fig = plt.figure(figsize=(8, 8))
			...
			if add_mean_fractional_dispersion == 1:
				...
				mean_fractional_dispersion_in_dex = np.log10(1.+mean_fractional_dispersion)
				plt.annotate("Mean fractional dispersion for %d < r < %d kpc = %.3f (%.3f dex) " % (mean_dispersion_radius_range[0], mean_dispersion_radius_range[1], mean_fractional_dispersion, mean_fractional_dispersion_in_dex), xy=(0.35, 0.08), size=10, xycoords='axes fraction')
			plt.savefig("star_pos_vel_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
			plt.clf()
		if draw_star_pos_vel_PDF >= 3:
			plt.clf()
#			plt.subplot(111, aspect=0.064)
			...
			if add_mean_fractional_dispersion == 1:
				...
				mean_fractional_dispersion_in_dex = np.log10(1.+mean_fractional_dispersion)
				plt.annotate("Mean fractional dispersion for %d < r < %d kpc = %.3f (%.3f dex) " % (mean_dispersion_radius_range[0], mean_dispersion_radius_range[1], mean_fractional_dispersion, mean_fractional_dispersion_in_dex), xy=(0.35, 0.08), size=10, xycoords='axes fraction')
			plt.savefig("star_pos_disp_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
			plt.subplot2grid((5, 1), (4, 0), rowspan=1)
			for code in range(len(codes)):
...
#			plt.subplot(111, aspect=0.064)
			...
			if add_mean_fractional_dispersion == 1:
				...
				mean_fractional_dispersion_in_dex = np.log10(1.+mean_fractional_dispersion)
				plt.annotate("Mean fractional dispersion for %d < r < %d kpc = %.3f (%.3f dex) " % (mean_dispersion_radius_range[0], mean_dispersion_radius_range[1], mean_fractional_dispersion, mean_fractional_dispersion_in_dex), xy=(0.35, 0.08), size=10, xycoords='axes fraction')
			plt.savefig("star_pos_disp_vert_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
			plt.clf()
	if draw_rad_height_PDF >= 1:
		fig_rad_height_PDF[time].savefig("rad_height_PDF_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
		if draw_rad_height_PDF == 2:
			plt.clf()
#			plt.subplot(111, aspect=18)
			...
			if add_mean_fractional_dispersion == 1:
				...
				mean_fractional_dispersion_in_dex = np.log10(1.+mean_fractional_dispersion)
				plt.annotate("Mean fractional dispersion for %d < r < %d kpc = %.3f (%.3f dex) " % (mean_dispersion_radius_range[0], mean_dispersion_radius_range[1], mean_fractional_dispersion, mean_fractional_dispersion_in_dex), xy=(0.35, 0.08), size=10, xycoords='axes fraction')
			plt.savefig("rad_height_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
			plt.clf()
	if draw_metal_PDF == 1:
		fig_metal_PDF[time].savefig("metal_PDF_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
	if draw_density_DF >= 1:
		plt.clf()
#		plt.subplot(111, aspect=1)
		...
		plt.savefig("density_DF_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
		plt.clf()
		if draw_density_DF == 2:
			fig = plt.figure(figsize=(8, 8))
			...
			plt.axvline(x = 1.67e-24*10, color='k', linestyle ='--', linewidth=2, alpha=0.7) # SF threshold density
			plt.grid(True)
			plt.savefig("density_DF_change_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
			plt.clf()
	if draw_radius_DF == 1:
		plt.clf()
#		plt.subplot(111, aspect=1)
		...
		ltext = leg.get_texts()
		plt.setp(ltext, fontsize='small')
		plt.savefig("radius_DF_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
...
#		plt.subplot(111, aspect=1)
		...
		if add_mean_fractional_dispersion == 1:
			...
			mean_fractional_dispersion_in_dex = np.log10(1.+mean_fractional_dispersion)
			plt.annotate("Mean fractional dispersion for %d < r < %d kpc = %.3f (%.3f dex) " % (mean_dispersion_radius_range[0], mean_dispersion_radius_range[1], mean_fractional_dispersion, mean_fractional_dispersion_in_dex), xy=(0.35, 0.08), size=10, xycoords='axes fraction')
		plt.savefig("gas_surface_density_radial_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
		plt.clf()
	if draw_star_radius_DF >= 1:
		plt.clf()
#		plt.subplot(111, aspect=1)
		...
		ltext = leg.get_texts()
		plt.setp(ltext, fontsize='small')
		plt.savefig("star_radius_DF_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
...
#		plt.subplot(111, aspect=1)
		...
		plt.savefig("star_surface_density_radial_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
		plt.clf()
...
#			plt.subplot(111, aspect=1)
			...
			# 	mean_fractional_dispersion_in_dex = np.log10(1.+mean_fractional_dispersion)
			#       plt.annotate("Mean fractional dispersion for %d < r < %d kpc = %.3f (%.3f dex) " % (mean_dispersion_radius_range[0], mean_dispersion_radius_range[1], mean_fractional_dispersion, mean_fractional_dispersion_in_dex), xy=(0.02, 0.84), size=10, xycoords='axes fraction')
			plt.savefig("sfr_surface_density_radial_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
			plt.clf()
...
#			plt.plot(Bigiel_surface_density, Bigiel_sfr_surface_density, 'b-', linewidth = 0.7, alpha = 0.4) 
			plt.plot(t, 1.37*t - 3.78, 'k--', linewidth = 2, alpha = 0.7) # observational fit by Kennicutt et al. 2007
			plt.savefig("K-S_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
			for code in range(len(codes)):
			...
			plt.savefig("K-S_with_fits_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
			plt.clf()
	if draw_height_DF == 1:
		plt.clf()
#		plt.subplot(111, aspect=0.5)
		...
		ltext = leg.get_texts()
		plt.setp(ltext, fontsize='small')
		plt.savefig("height_DF_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
...
#		plt.subplot(111, aspect=0.8)
		...
		plt.savefig("gas_surface_density_vertical_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
		plt.clf()
	if draw_SFR >= 1:
		plt.clf()
#		plt.subplot(111)#, aspect=1e-7)
		...
		ltext = leg.get_texts()
		plt.setp(ltext, fontsize='small')
		plt.savefig("Stellar_mass_evolution_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
		plt.clf()
#		plt.subplot(111, aspect=55)
		...
		plt.savefig("SFR_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
		plt.clf()
	if draw_cut_through == 1:
		plt.clf()
#		plt.subplot(111, aspect=0.5)
		...
		plt.savefig("cut_through_z_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
		plt.clf()
#		plt.subplot(111, aspect=0.5)
		fig = plt.figure(figsize=(8, 6))
		for code in range(len(codes)):
		...
		plt.savefig("cut_through_x_z%d" % times[time], bbox_inches='tight', pad_inches=0.03, dpi=300)
		plt.clf()
