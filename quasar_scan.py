...
class QuasarSphere(GeneralizedQuasarSphere):
    ...
    def get_ion_column_num(self,ion,frac=None):
        intensivesdict = {'Z':-1,'n':-2,'T':-3}
        if frac == None:
            plus = 0
        if ion == "Z":
    ...
    def save_values(self,dest = None):
        ...
        fifthline = "ions,ion_frac,%s,T,n,Z\n"%str(gasbins.get_all_bintypes()).strip("[]").replace("'","").replace('"',"")
...
def read_values(filename):
    """ firstline = "[dsname, z, center[0], center[1], center[2], Rvir, pathname]\n"
        ...
        thirdline = "[R, n_th, n_phi, n_r, r_max, num_lines, line_reached]\n"
        fourthline = str(self.scanparams)+"\n"
        fifthline = "ions,ionfrac,bintypes,T,n,Z\n"
        sixthline = "["+str(self.ions[0])+", "+...+"]"
    """
    f = open(filename)
    ...
    savebins = fifthline.split("ion_frac,")[1].split(",T,n")[0].split(",")
    gasbins = gasbinning.GasBinsHolder(savebins)
